...
interface PspdfkitWidgetControllerApi {
   ...
   */
  fun enterAnnotationCreationMode(authorName: String, callback: (Result<Boolean?>) -> Unit)
  /**
   ...
   */
  fun jumpToPage(pageIndex: Long, callback: (Result<Boolean>) -> Unit)
  /**
   ...
   */
  fun isShowingTwoPages(callback: (Result<Boolean>) -> Unit)
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: PspdfkitWidgetControllerApi?, messageChannelSuffix: String = "") {
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val authorNameArg = args[0] as String
            api.enterAnnotationCreationMode(authorNameArg) { result: Result<Boolean?> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.pspdfkit_flutter.PspdfkitWidgetControllerApi.jumpToPage$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pageIndexArg = args[0] as Long
            api.jumpToPage(pageIndexArg) { result: Result<Boolean> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.pspdfkit_flutter.PspdfkitWidgetControllerApi.isShowingTwoPages$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            api.isShowingTwoPages{ result: Result<Boolean> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
  ...
  fun onDocumentSaved(documentIdArg: String, pathArg: String?, callback: (Result<Unit>) -> Unit)
{
  ...
  fun onAnnotationCreationModeExited(callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.pspdfkit_flutter.PspdfkitWidgetCallbacks.onAnnotationCreationModeExited$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(null) {
      if (it is List<*>) {
        if (it.size > 1) {
