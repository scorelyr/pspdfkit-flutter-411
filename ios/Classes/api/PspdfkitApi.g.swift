...
protocol PspdfkitApi {
  ...
  func addAnnotation(jsonAnnotation: String, completion: @escaping (Result<Bool?, Error>) -> Void)
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: PspdfkitApi?, messageChannelSuffix: String = "") {
    ...
    if let api = api {
      addAnnotationChannel.setMessageHandler { message, reply in
        ...
        api.addAnnotation(jsonAnnotation: jsonAnnotationArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
...
protocol PspdfkitWidgetControllerApi {
  ...
  func enterAnnotationCreationMode(authorName: String, completion: @escaping (Result<Bool?, Error>) -> Void)
  ...
  func jumpToPage(pageIndex: Int64, completion: @escaping (Result<Bool, Error>) -> Void)
  ...
  func isShowingTwoPages(completion: @escaping (Result<Bool, Error>) -> Void)
}
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: PspdfkitWidgetControllerApi?, messageChannelSuffix: String = "") {
    ...
    if let api = api {
      enterAnnotationCreationModeChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let authorNameArg = args[0] as! String
        api.enterAnnotationCreationMode(authorName: authorNameArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
    ...
    /// pageIndex The index of the page to jump to. This is a zero-based index.
    /// Returns a [Future] that completes with true if the jump was successful, false otherwise.
    let jumpToPageChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.pspdfkit_flutter.PspdfkitWidgetControllerApi.jumpToPage\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      jumpToPageChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let pageIndexArg = args[0] as! Int64
        api.jumpToPage(pageIndex: pageIndexArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
      ...
      jumpToPageChannel.setMessageHandler(nil)
    }
    /// Determines if the document is currently showing two pages or not. Useful when setting the initial page layout to auto
    /// Returns a [Future] that completes with true if the document is showing two pages, false otherwise.
    let isShowingTwoPagesChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.pspdfkit_flutter.PspdfkitWidgetControllerApi.isShowingTwoPages\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      isShowingTwoPagesChannel.setMessageHandler { _, reply in
        api.isShowingTwoPages { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
      ...
      isShowingTwoPagesChannel.setMessageHandler(nil)
    }
  }
}
/// Generated protocol from Pigeon that represents a handler of messages from Flutter.
...
protocol PspdfkitWidgetCallbacksProtocol {
  ...
  func onAnnotationCreationModeExited(completion: @escaping (Result<Void, PspdfkitApiError>) -> Void)
}
class PspdfkitWidgetCallbacks: PspdfkitWidgetCallbacksProtocol {
  ...
  func onAnnotationCreationModeExited(completion: @escaping (Result<Void, PspdfkitApiError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.pspdfkit_flutter.PspdfkitWidgetCallbacks.onAnnotationCreationModeExited\(messageChannelSuffix)"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
    channel.sendMessage(nil) { response in
      guard let listResponse = response as? [Any?] else {
        completion(.failure(createConnectionError(withChannelName: channelName)))
